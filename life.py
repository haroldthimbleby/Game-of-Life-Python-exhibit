...
historyDepth = 5
gridHistory = [[[0 for col in range(fullHeight)] for row in range(fullWidth)] for i in range(historyDepth)]
...
def equalGrids(a): # only compare what the user can see (exclude the borders)
    global grid
    if a == grid: return False  # don't test a==grid
...
def isRepeated(): # NB this won't detect cycles longer than {historyDepth-1}
    ...
    for i in range(historyDepth):
        if equalGrids(gridHistory[i]): return True
    return False

def say(str, centre, color, font=boldFont):
...
def explain(str, topleft, widthheight, font=boldFont, edge=True):
    round_rect(screen, pygame.Rect(screenCoords(topleft[0]-.7+border, topleft[1]+border),
                                   ...
                                   edge=edge)
    say(str, (topleft[0]-.5, topleft[1]+.3), explainColor, font=font)
...
def exp(n, theExplanation):
    ...
    if explainSteps[n]:
        explainSteps[n] = explainSteps[n]-1; explainCount[n] = explainCount[n]+1
        theExplanation()
...
def explanations():
    ...
    if explainGlider(28, 13): explainCount[8] = explainCount[8]+1       
    if explainGlider(32, 17): explainCount[9] = explainCount[9]+1
    if explainGlider(38, 23): explainCount[10] = explainCount[10]+1
    
def plot(explain=False):
    drawblankboard()
    drawgrid()
...
def nextgrid():
    ...
    for y in range(1, fullHeight-1):
        for x in range(1, fullWidth-1):
            ...
            else: ngrid[x][y] = 1 if n == 3 else 0
            ...
            while True:
                ...
                for event in events:
                    if event.type == pygame.KEYDOWN:
                        print(f"Stopped by keydown event")
                        fullbreak = True
                        break
                if fullbreak: break
                if randomised:
